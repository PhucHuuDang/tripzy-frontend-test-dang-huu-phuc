...
export const Button = ({
  ...
  return (
    <button
      ...
      tabIndex={0}
      className={cn(
        "flex justify-center items-center h-full gap-0.5 duration-150",
        sizes[+svgOnly][size],
        disabled || loading
          ? "bg-gray-100 text-gray-700 border h-full border-gray-400 cursor-not-allowed"
          : types[type],
        shapes[shape][size],
        shadow && "shadow-border-small border-none",
        fullWidth && "w-full",
        variant === "unstyled"
          ? "outline-none px-0 h-full bg-transparent hover:bg-transparent  rounded-sm  text-gray-1000"
